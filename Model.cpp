...
void Model::LoadModel(const std::string & fileName)
{
	...
	if (!scene)
	{	
		printf("No se pudo cargar el modelo, verifica la carpeta de Modelos: %s %s\n", fileName.c_str(), importer.GetErrorString());
		return;
	}
	...
	for (unsigned int i = 0; i < scene ->mNumMaterials; i++)
	{
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE	))
		{
			...
			if (material->GetTexture(aiTextureType_DIFFUSE,0,&path)==AI_SUCCESS)
			{
				...
				std::string texPath = std::string("Texturas/") + filename;
		...
		if (!TextureList[i])
		{
			TextureList[i] = new Texture("Texturas/plain.png"); //textura que se aplicará a los modelos si no tienen textura o la textura no se puede cargar
